...
video = cv2.VideoWriter("original.avi", fourcc, 30.0, (frameWidth, frameHeight))
summarised_video = cv2.VideoWriter("summarised.avi", fourcc, 30.0, (frameWidth, frameHeight))
...
while True:
    try:
        ...
        if detection_result_list:
            categories = [category.category_name for category in [detection.categories[0] for detection in detection_result_list[0].detections]]
            if "cell phone" in categories:
                summarised_video.write(frame)
