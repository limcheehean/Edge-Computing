...
while True:
    try:
        ...
        for idx in range(len(hand_landmarks_list)):
            ...
            for _ in range(21):
                x = int(hand_landmarks[_].x * frame.shape[1])
                y = int(hand_landmarks[_].y * frame.shape[0])
                cv2.circle(frame, (x, y), 5, (0, 255, 0), -1)

            # y increases downwards!!!
            # Check position of THUMB_CMC and THUMB_IP relative to WRIST
            # If THUMB_IP is closer and lower, then finger is up
            wrist, thumb_cmc, thumb_tip = hand_landmarks[0], hand_landmarks[1], hand_landmarks[4]
            if abs(wrist.x - thumb_cmc.x) < abs(wrist.x - thumb_tip.x) and thumb_cmc.y > thumb_tip.y:
                fingers_up += 1

            # Loop through the other fingers
            for _ in range(5, 21, 4):
                finger_up = 1
                # Check if each landmark is lower than previous
                for landmark in range(_ + 1, _ + 4):
                    # Finger down if landmark is higher than previous
                    if hand_landmarks[landmark].y > hand_landmarks[landmark - 1].y:
                        finger_up = 0
                fingers_up += finger_up

            # # Define a threshold for thumb is up/down and display when thums up
            # threshold = 0.1
            # thumb_tip_y = hand_landmarks[4].y
            # thumb_base_y = hand_landmarks[
            #     1].y  # Index 1 corresponds to the thump base as from https://developers.google.com/mediapipe/solutions/vision/hand_landmarker
            # thums_up = thumb_tip_y < thumb_base_y - threshold
            #
            # if thums_up:
        cv2.putText(frame, f"{fingers_up} Fingers Up", (10, 30),
                    cv2.FONT_HERSHEY_DUPLEX,
                    FONT_SIZE, HANDEDNESS_TEXT_COLOR, FONT_THICKNESS, cv2.LINE_AA)

        cv2.imshow('Number of fingers up', frame)
